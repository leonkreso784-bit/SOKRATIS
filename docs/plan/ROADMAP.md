...
`explain` id-eva, dimni test) usporedno s **T64** (tok LANCI, S-038 — nespojene grane kao lanci, ne
popis), pa tok IZDANJE (T35, T63, T43) i 1.0.0.

**Na disku dva stabla:** `sokratis` (`main`, vrh `93f1180`) i `sokratis.rel`
(`feat/release`, T35 napola — pet datoteka necommitano, bez izvještaja) — parkirano do toka IZDANJE,
nova sesija ga ne dira ni ne briše. Sva stabla tokova sesija 1–4 (DESKTOP-2, JEZGRA-2, GRAFOVI, IO-2,
PLOČA) su obrisana nakon provjere da su potpuno spojena u pushani `main`.
...
kartica s objašnjenjem) su spojena u `main`; drugi rez do 1.0.0 (S-032…S-037) dodao je sesije 1–4
(DESKTOP-2, JEZGRA-2, GRAFOVI, IO-2, PLOČA) — mjerenje po zadanom profilu je sad preko **svih lokalnih
grana** (S-032), dnevnik unija svih radnih stabala (S-033), klik na karticu otvara nadzornu ploču
projekta (S-034), verzija u kodu je sad `1.0.0-pre.4`. Puna
