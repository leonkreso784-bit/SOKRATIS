...
| `let … else` | M1 (T3, `gitlog.rs`; kasnije `docs.rs`, `git.rs`); dopuna M2/52 (`io/tests/perf.rs::measure_real_repo_both_scopes`, rani `return` kad `SOKRATIS_MEASURE_REPO` nije postavljen) | rani izlazak (`return`/`continue`) kad uzorak ne odgovara, bez ugnježđenog `match` za jedan slučaj |
...
| `#[arg(long, value_parser = ["all", "default"])]` | M2/51 (`cli/src/main.rs`, `--scope`) | popis dopuštenih vrijednosti kao `value_parser` bez ručnog `match`-a nad greškom — nepoznata vrijednost postaje `clap`-ova greška uporabe (`try_parse` je već pretvara u izlaz 3, ne 2/ALERT, isti obrazac kao I4) |
...
| `fill-opacity` kao monotona ljestvica nad jednim tokenom | M2/56 (`src/app.css`, `.heat-0…4`, R57) | umjesto stepenica boje (`brand-400/500/600` nisu monotone na sve četiri teme) razine dijele JEDNU boju i razlikuju se samo prozirnošću (0,3 → 1) — prozirnost je monotona neovisno o temi |
