...
| let-chain (`if let PATTERN = IZRAZ && UVJET`) | M2/3 (`report.rs`, provjera oblika `until`); dopuna M2/11 (`io/src/git.rs::last_changes`, spajanje retka `@@<unix>` s pripadnim putanjama u jednoj provjeri) | stabilno od edition 2024: `let`-uzorak i dodatni bool-uvjet u istom `if`, bez ugniježđenog `if` unutar `if let` |
