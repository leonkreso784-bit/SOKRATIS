...
//! pamti commite; ovaj `impl` na `StoreCache` je JEDINO mjesto koje ih spaja. Detalj (zaključavanje,
//! zamka reentrantnosti) je uz `StoreCache` i `StoreCache::lock` niže.
...
/// — ovaj adapter ga ne dira, samo prosljeđuje `project_id` storeu. Drži `&Mutex<Store>` (ne
/// `Store` izravno) i zaključava ga PRI SVAKOM POZIVU — kratko, samo unutar `cached`/`store`, nikad
/// dulje. Zato pozivatelj (`compute` u `commands.rs`) NE SMIJE držati bravu storea dok zove
/// `Project::input_cached`: std `Mutex` nije reentrantan, drugi `lock()` iz iste niti bi se
/// zauvijek blokirao.
