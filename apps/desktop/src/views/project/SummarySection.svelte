<script lang="ts">
  ...
  // Krug popravka 1 (vizualni dimni test na 1048 px): `xl:grid-cols-6` (ne `lg:`) ostavlja kartice
  // dovoljno široke da najdulji mogući tekst signala (npr. „2 uzbune · 1 upozorenje") stane bez
  // vodoravnog klizača `<main>`-a; `min-w-0` na kartici + `break-words` na vrijednosti su druga
  // linija obrane, a kartica signala nosi manji `text-xl` jer joj je tekst redovito najduži.
...
<div class="flex flex-col gap-4">
  ...
  <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
    <div class="flex min-w-0 flex-col gap-2 rounded-lg border border-line bg-surface-1 p-4 shadow-e1">
      <span class="text-xs text-ink-2">{t('summary.commits')}</span>
      <p class="break-words text-2xl font-semibold text-ink-0">
    ...
    <div class="flex min-w-0 flex-col gap-2 rounded-lg border border-line bg-surface-1 p-4 shadow-e1">
      ...
      <p class="break-words text-xl font-semibold text-ink-0">
