...
<script lang="ts">
  import { arcPath, ringSegments } from './scale';
  ...
  const name = $derived(label ?? segments.map((s) => s.label).join(', '));
</script>

<svg viewBox="0 0 {size} {size}" role="img" aria-label={name} class="w-full">
  {#each segments as s, i (i)}
    {@const seg = shares[i]}
    {#if seg}
      <path d={arcPath(cx, cy, r, seg.start, seg.end)} stroke={s.color} stroke-width="18" fill="none">
        <title>{s.label}: {Math.round(seg.share * 100)}%</title>
      </path>
    {/if}
  {/each}
</svg>
