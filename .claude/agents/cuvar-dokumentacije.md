...
3. **`docs/workflow/RUST.md` §4 pojmovnik** — iz „NOVI RUST-POJMOVI" u izvještajima i iz zaglavlja `//! ZAŠTO RUST OVAKO` (pročitaj ih: `grep -rn "ZAŠTO RUST OVAKO" crates/`): svaki nov pojam dobiva redak *pojam · prvi put (cigla, datoteka) · jedna rečenica*. Postojeći redak s „*kad se pojavi*" se dopuni, ne duplicira.
